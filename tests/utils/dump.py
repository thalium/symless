...
def dump_structures() -> dict:
    ...
    for idx, sid, name in idautils.Structs():
        original_cmt = idaapi.get_struc_cmt(sid, False)
        if original_cmt is None:
            order = 0
        elif ":" in original_cmt:
            print(original_cmt)
            print(original_cmt.split(":")[1])
            print(original_cmt.split(":")[1].strip().split(" ")[0])
            order = int(original_cmt.split(":")[1].strip().split(" ")[0].split(",")[0], 16)
        else:
            order = 0
        structures.append((order, name, sid, original_cmt))
    structures.sort()

    for order, name, sid, original_cmt in structures:
        ...
        report("struc comment %s:" % original_cmt)
        report("struc %s:" % name)
